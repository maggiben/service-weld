...
| ------------- | --------------------------------------------------------------------------------------------------------- |
| Before commit | `pnpm run check:secrets`, `pnpm run check:deps`, Prettier (staged / `format:check`), `pnpm run typecheck` |
| Before push   | `pnpm run test:coverage` — **≥80%** lines/branches/functions/statements on **every** workspace package    |

Hooks: `.husky/pre-commit`, `.husky/pre-push`. Coverage script: `scripts/check-coverage.mjs` (default threshold `80`). Deps script: `scripts/check-deps.mjs`. Specs: `010` R9–R10, `012` R8, D-16.
