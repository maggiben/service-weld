...
- R6. Implement **CI/CD**: run the full **Vitest** suite (`010`) incl. schema load + invariant tests, format/typecheck, unit tests, and the **≥80% global coverage gate** (`pnpm run test:coverage`); apply DDL migrations on deploy; zero-downtime rollout where possible.
